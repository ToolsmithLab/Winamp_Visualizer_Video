...
TimeKeeper::TimeKeeper(double presetDuration, double smoothDuration, double hardcutDuration, double easterEgg)
    ...
    , m_deterministic(DeterministicState::Enabled())
...
void TimeKeeper::UpdateTimers(uint32_t targetFps)
{
    if (m_deterministic)
    {
        m_secondsSinceLastFrame = 1.0 / static_cast<double>(std::max<uint32_t>(1U, targetFps));
