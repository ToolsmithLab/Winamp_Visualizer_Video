...
auto TextureManager::GetRandomTexture(const std::string& randomName) -> TextureSamplerDescriptor
{
    ...
    std::default_random_engine rndEngine(DeterministicState::NextSeed32());
...
void TextureManager::ScanTextures()
{
    if (!m_filesScanned)
    {
        ...
        std::sort(
            m_scannedTextureFiles.begin(),
            m_scannedTextureFiles.end(),
            [](const ScannedFile& left, const ScannedFile& right) {
                if (left.lowerCaseBaseName != right.lowerCaseBaseName)
                {
                    return left.lowerCaseBaseName < right.lowerCaseBaseName;
                }
