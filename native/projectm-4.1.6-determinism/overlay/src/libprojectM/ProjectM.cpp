...
void ProjectM::RenderFrame()
{
    ...
    m_timeKeeper->UpdateTimers(m_targetFps);
...
void ProjectM::Initialize()
{
    ...
    /* The seed is configured before construction by the C API. */
    srand(DeterministicState::NextSeed32());
...
void ProjectM::StartPresetTransition(std::unique_ptr<Preset>&& preset, bool hardCut)
{
    ...
    else
    {
        ...
        m_transition = std::make_unique<Renderer::PresetTransition>(
            m_transitionShaderManager->RandomTransition(),
            m_softCutDuration,
            DeterministicState::Enabled());
    }
}
