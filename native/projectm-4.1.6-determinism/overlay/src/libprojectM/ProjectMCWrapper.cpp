...
projectm_handle projectm_create()
{
    try
    {
        ...
        const auto clockSeed = static_cast<uint64_t>(
            std::chrono::high_resolution_clock::now().time_since_epoch().count());
        const auto seed = clockSeed ^ (static_cast<uint64_t>(entropy()) << 32U) ^
                          static_cast<uint64_t>(entropy());
        libprojectM::DeterministicState::Configure(seed, false);
...
projectm_handle projectm_create_with_seed(uint64_t seed)
{
    try
    {
        libprojectM::DeterministicState::Configure(seed, true);
        auto projectMInstance = new libprojectM::projectMWrapper();
        return reinterpret_cast<projectm_handle>(projectMInstance);
    }
    catch (...)
