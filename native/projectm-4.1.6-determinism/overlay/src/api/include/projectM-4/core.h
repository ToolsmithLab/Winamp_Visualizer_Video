...
/**
 * @brief Creates a projectM instance with deterministic random state and time.
 *
 * The seed is applied before any projectM resource or idle preset is created.
 * Rendering time advances by exactly 1/fps for each rendered frame.
 *
 * @param seed Unsigned 64-bit deterministic seed.
 * @return A projectM handle, or NULL if creation failed.
 */
PROJECTM_EXPORT projectm_handle projectm_create_with_seed(uint64_t seed);
