...
static uint32_t prjm_eval_mt[N];
static int32_t prjm_eval_mti;

void prjm_eval_set_random_seed(uint32_t seed)
{
    prjm_eval_mt[0] = seed & 0xffffffffUL;
    for (prjm_eval_mti = 1; prjm_eval_mti < N; prjm_eval_mti++)
    {
        prjm_eval_mt[prjm_eval_mti] =
            (1812433253UL *
                 (prjm_eval_mt[prjm_eval_mti - 1] ^
                  (prjm_eval_mt[prjm_eval_mti - 1] >> 30)) +
             prjm_eval_mti);
        prjm_eval_mt[prjm_eval_mti] &= 0xffffffffUL;
    }
}
...
static uint32_t prjm_eval_genrand_int32(void)
{
    ...
    if (!prjm_eval_mti)
    {
        prjm_eval_set_random_seed(0x4141f00d);
    }
    ...
    { /* generate N words at one time */
        ...
        for (kk = 0; kk < N - M; kk++)
        {
            y = (prjm_eval_mt[kk] & UPPER_MASK) | (prjm_eval_mt[kk + 1] & LOWER_MASK);
            prjm_eval_mt[kk] = prjm_eval_mt[kk + M] ^ (y >> 1) ^ mag01[y & 0x1UL];
        }
        for (; kk < N - 1; kk++)
        {
            y = (prjm_eval_mt[kk] & UPPER_MASK) | (prjm_eval_mt[kk + 1] & LOWER_MASK);
            prjm_eval_mt[kk] = prjm_eval_mt[kk + (M - N)] ^ (y >> 1) ^ mag01[y & 0x1UL];
        }
        y = (prjm_eval_mt[N - 1] & UPPER_MASK) | (prjm_eval_mt[0] & LOWER_MASK);
        prjm_eval_mt[N - 1] = prjm_eval_mt[M - 1] ^ (y >> 1) ^ mag01[y & 0x1UL];

        prjm_eval_mti = 0;
    }

    y = prjm_eval_mt[prjm_eval_mti++];
